...
public class Md5 {
	...
	public String stringify(byte buf[]) {
	...
	/**
	 * 功能：返回md5加密后的密文  add by yanghaibo 2012-11-15 13:23
	 * @param base64   经过base64加密后的明文密码
	 * @return
	 * @throws NoSuchAlgorithmException
	 */
	public static String base64ToMd5(String base64) throws NoSuchAlgorithmException{
		String password = base64.trim();
		MessageDigest md5Pwd = MessageDigest.getInstance("md5");
		md5Pwd.update(password.getBytes());
		byte[] zz = md5Pwd.digest();
		StringBuffer sb = new StringBuffer();
		for ( int i =0;i<zz.length ;i++){
			if((zz[i] & 0xff) < 0x10){
				sb.append("0");
			}
			sb.append(Long.toString(zz[i] & 0xff,16));
		}
		return sb.toString();
	}
}